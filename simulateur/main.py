    ...
    for i in range(steps):
        ...
        for r in robots:
            if r.has_lidar:
                (robot_x, robot_y) = r.get_center()
                robot_r = r.get_rotation()
                rad_data = get_lidar_data(robot_x, robot_y, robot_r, obstacles, i*LIDAR_ANGLE_PER_STEP, (i+1)*LIDAR_ANGLE_PER_STEP)
                # rad_data = lidar.get_one_scan()
                r.update_lidar(rad_data, True, i*LIDAR_ANGLE_PER_STEP)
